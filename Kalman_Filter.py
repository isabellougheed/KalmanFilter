...
class Kalman_Filter:
    ...
    def predict(self, x_prior, P_prior, a):
        ...
        Pk_p = 0.5 * (Pk_p + Pk_p.T)  # force symmetry
        return xk_p, Pk_p
        ...
        for i in range(0, steps - 1):
            ...
            if frequ_a != frequ_y:
                ...
                if i % (frequ_a / frequ_y) == 0:
                    ...
                    x_k = xk_p + K_k * (
                        y_n[i] - C @ xk_p
                    )  # x_k+1|k+1 = x_k+1|k + K_k+1(y_k+1 - (C_k+1)(x_k+1|k))
                    P_k = (np.eye(2, 2) - K_k @ C.reshape(1, -1)) @ Pk_p @ (
                        np.eye(2, 2) - K_k @ C.reshape(1, -1)
                    ).T + K_k * R_d * K_k.T  # P_k+1|k+1 = (I - (K_k+1)(C_k+1))P_k+1|k(I - (K_k+1)(C_k+1)).T + (K_k+1)(R_k+1)(K_k+1).T
                    P_k = 0.5 * (P_k + P_k.T)  # force symmetry
...
ax[1].set_ylabel(r"$\dot{r}(t)$ (m/s)")
ax[2].set_ylabel(r"$\ddot{r}(t)$ (m/$s^2$)")
# Plot data
ax[0].plot(t_sol, r_sol, label="$x_1(t)$", color="C0")
ax[1].plot(t_sol, dotr_sol, label="$x_2(t)$", color="C0")
ax[2].plot(t_sol, a_sol, label="$x_2(t)$", color="C0")
fig.tight_layout()
plt.show()
...
ax[1].set_ylabel(r"$\dot{r}(t)$ (m/s)")
ax[2].set_ylabel(r"$\ddot{r}(t)$ (m/$s^2$)")
ax[3].set_ylabel(r"$u(t)$ (N)")
# Plot data
ax[0].plot(t_sol2, r_sol2, label="$x_1(t)$", color="C0")
ax[1].plot(t_sol2, dotr_sol2, label="$x_2(t)$", color="C0")
ax[2].plot(t_sol2, a_sol2, label="$x_2(t)$", color="C0")
ax[3].plot(t_sol2, u, label="$u(t)$", color="C0")
fig.tight_layout()
plt.show()
...
ax[0].fill_between(
    t_steps, sigma3_1_2, -sigma3_1_2, label=r"$3\sigma_1$", color="lightblue"
)
ax[1].plot(t_steps, error_x2_2, label="estimated velocity", color="C0")
ax[1].fill_between(
    t_steps, sigma3_2_2, -sigma3_2_2, label=r"$3\sigma_2$", color="lightblue"
)
